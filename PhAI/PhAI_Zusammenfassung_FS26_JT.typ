...
#hinweis[
  *Hinweis:* Die Anweisungen zur Taschenrechnerbenutzung beziehen sich auf den TI nSpire CX II-T CAS.
]
...
In einem Dreieck kann eine Linie gezogen werden. Diese verkleinert das Dreieck.\ Die neue Grösse kann mit dem
Strahlensatz berechnet werden.
...
#definition[
  #table(
    ...
    [
      $ dif/(dif x) (f(x) dot g(x)) =\ f'(x) dot g(x) + f(x) dot g'(x) $
    ],
    [
      $ dif/(dif x) (f(g(x))) =\ f'(g(x)) dot g'(x) $
    ],
    [
      $ dif/(dif x) (g(x)/h(x)) =\ (g'(x) dot h(x) - g(x) dot h'(x)) / ((h(x))^2) $
    ],
  )
]
...
#{
  ...
  table(
    ...
    [*$f(x)$*], [1], [$x^n$], [$1 / x "/" x^(-1)$], [$sqrt(x) "/" x^(1/2)$], [$sin(x)$], [$cos(x)$], [$e^x$], [$ln(x)$],
...
#hinweis[TR: Wie normale Ableitung (Menü #sym.arrow 4 #sym.arrow 1), aber es müssen explizit Mal-Zeichen gesetzt werden.
  Wenn nach einem Winkel abegleitet wird ($alpha$), fügt der TR gerne noch $pi$ und $180 dot$ hinzu. Diese Angaben
...
#definition[
  $
    f = f(x, y), space x = x(t), space y = y(t) quad => quad
    (dif f)/(dif t) = (partial f)/(partial x) dot (dif x)/(dif t) + (partial f)/(partial y) dot (dif y)/(dif t)
  $
]

Totale Ableitungen nach der Zeit $t$ werden in der Physik oft als $dot(x)$ geschrieben; der Punkt bedeutet also _"totale
Ableitung nach der Zeit"_.
#definition[
  $ dot(F) = (dif F)/(dif t) quad quad dot.double(F) = (dif F)/(dif t)((dif F)/(dif t)) $
]

*Beispiel:* $"Gegeben:" f(x, y) &= x^2 dot e^y + y^2 - x, quad space x(t) = sin(t), quad space y(t) = t^2$
$
  x(t) = sin(t) => (dif x)/(dif t) & = dot(x) = cos(t) quad quad quad
                                     y(t) = t^2 => (dif y)/(dif t) & = dot(y) = 2t
$
$
  (dif f)/(dif t) & = (partial f)/(partial x) dot (dif x)/(dif t) + (partial f)/(partial y) dot (dif y)/(dif t) \
...
== Linearisierung
Mit der Linearisierung kann ein Punkt auf einer Funktion mit einer Tangente angenähert werden. Funktionen mit zwei oder
mehr Variablen müssen partiell abgeleitet werden.
#definition(
  grid(
    align: horizon,
    $
...
#example-block[
  ...
  #grid(
    ...
    [
      $
        1) space (partial f)/(partial x) & = 2x dot sin(y), quad (partial f)/(partial y) = x^2 dot cos(y) \
...
*Beispiel:*
$ "Gesucht:" f(0.9, 0.1) \ "Gegeben:" f(x, y) = x^2 dot sin(y) "linearisiert mit" x = 1, y = 0, quad Delta f = Delta y $
#v(-0.5em)
#grid(
  [
    $
      Delta x & = x - 1 = 0.9 - 1 = -0.1 \
      Delta y & = y - 0 = 0.1 - 0 = 0.1 \
      Delta f & = Delta y = 0.1
    $
  ],
  [
    $
      "weil" x = 0.1 approx 1, y = 0.1 approx 0\
      f = f(1, 0) = + Delta f = 1^2 dot sin(0) + 0.1 = underline(0.1)
    $
  ],
)
...
- Mit der Kraft von $1"N"$ kann ein ruhender Körper mit Masse $1"kg"$ innerhalb von $1"s"$ auf $1"m"\/"s"$ beschleunigt
  werden kann.
- Verdoppelt man die Masse oder die Beschleunigung, verdoppelt sich auch die benötigte Kraft -- die Kraft ist
  proportional zur Masse und zur Geschwindigkeit
- Halbiert man die Zeit, verdoppelt sich die benötigte Kraft -- die Kraft ist umgekehrt proportional zur Dauer

=== Umrechnungen
_Achtung!_ In der Prüfung sind die Einheiten oft nicht in SI-Einheiten angegeben! Einfachheitshalber sollten sie in
diese konvertiert werden.\
...
- _Systematischer Fehler:_ Fehler durch ungenaue Messgeräte oder den Experimentaufbau. Verschwindet bei wiederholtem
  Messen nicht.
- _Statistischer Fehler:_ Fehler durch den Experimentator. Wird durch wiederholtes Messen und Mittelwertbildung
  verringert.
...
#grid(
  ...
  $ l = (2.50 plus.minus 0.05)"m" quad => quad (2.45; 2.55)"m" $,
)
...
- Der Messfehler hat _genau so viele Dezimalstellen_ wie der Messwert.\ $(2.5 + 0.05)"m"$ verstösst beispielsweise gegen
  diese Regel #hinweis[($10^(-1)$ vs. $10^(-2)$)].
- Die Anzahl der signifikanten Stellen im Ergebnis einer Multiplikation oder Division ist nie grösser als die der Grösse
  mit den wenigsten signifikanten Stellen:\ $1.2 dot 0.0315 = 0.03cancel(78) = 0.38$
- Die Anzahl der Dezimalstellen bei einer Addition oder Subtraktion mehrerer Grössen entspricht der des Terms mit der
  kleinsten Anzahl von Dezimalstellen:\ $1.040 + 1.210415 = 2.250cancel(415) = 2.250$
...
_Achtung! Ist eine Messgrösse in Grad angegeben, den TR in den Grad-Modus schalten_ #hinweis[(Oben rechts auf "RAD"
  klicken)] _und den Fehler vorher ins Bogenmass umrechnen!_
...
#definition[
  #grid(
    ...
    [
      _$bold(x_i)$_: Messgrösse ohne absolutem Fehler\
...
#hinweis[*TR:* Leider keine built-in Funktion. Am besten die gegebene Funktion als Variable speichern, damit sie für die
  partiellen Ableitungen wiederverwendet werden kann: `err := x^2/sqrt(y)`. Partielle Ableitungen einzeln bestimmen,
  #tr-constructs-button drücken und $|square.stroked.dotted|$ auswählen. Die Partielle Ableitung kann von oben mit
  "Enter" kopiert werden. Delta einsetzen. Wiederholen, bis alle Variablen abgedeckt sind. Hinter der Rechnung können
  Variablen mit `| x=2 and y=3` substituiert werden.]
...
#example-block[
  ...
  #grid(
    ...
    [
      ...
      + Werte ohne Messfehler in $Delta f$ einfügen
      + Ausrechnen
    ...
    [
      $
              1) space Delta f & = abs((delta f)/(delta x)) dot Delta x + abs((delta f)/(delta y)) dot Delta y \
                         space & = abs((2x)/sqrt(y)) dot Delta x + abs(x^2/(2sqrt(y^3))) dot Delta y \
              2) space Delta f & = abs((2 dot 2.0)/sqrt(4.0)) dot 0.1 + abs(2.0^2/(2sqrt(4.0^3))) dot 0.5 \
...
die anderen $0$ sind. Einheitsvektoren können mithilfe des Betrags des Vektors bestimmt werden.

#grid(
  align: center + horizon,
  $ hat(x) = vec(1, 0, 0), hat(y) = vec(0, 1, 0), hat(z) = vec(0, 0, 1) $,
  $ hat(hk(r)) = hk(r)/(abs(hk(r))) $,
)

== Bahn eines Teilchens <bahn-teilchen>
Die _Länge eines Vektors $bold(abs(hk(r)))$_ wird mit dem Pythagoras berechnet. Damit ist es möglich, den _Abstand
zwischen zwei Teilchen_ zu erhalten.
$
  abs(hk(r)) = sqrt(r_x^2 + r_y^2 + r_z^2)\
  abs(hk(r_1) - hk(r_2)) = abs(hk(r_2) - hk(r_2))
  = sqrt((r_(1, x) - r_(2, x))^2 + (r_(1, y) - r_(2, y))^2 + (r_(1, z) - r_(2, z))^2)
$

Um die Bewegung eines Teilchens zu beschreiben, wird der Begriff _Bahn $bold(hk(r)(t))$_ verwendet. Die Funktion ordnet
dem Teilchen zu jedem Zeitpunkt $t$ seine Position zu. Mit der Startposition $hk(r)_A$ und der Endposition $hk(r)_E$
kann der _Verschiebungsvektor $bold(Delta hk(r))$_ gebildet werden, welcher die Verschiebung des Teilchens
#hinweis[(Luftlinie)] anzeigt.\
...
#grid(
  align: center,
  position-auf-kreis-cos-sin(),
  position-auf-kreis-sin-cos(),
)

#grid(
  [
    *Beispiel: Bewegung auf dem Kreis*\
    Die Bewegung auf dem Einheitskreis kann mit Sinus und Cosinus dargestellt werden. Ist der _Cosinus oben_, ist die
    Bewegung im _Gegen-Uhrzeigersinn_, wenn der _Sinus oben_ ist, im _Uhrzeigersinn_.
...
Start-/Endzeit ausgerechnet werden. Dadurch erhält man den _mittleren Geschwindigkeitsvektor
$bold((Delta hk(r))/(Delta t))$_.
...
Um die _Geschwindigkeit zum Zeitpunkt $bold(t)$_ zu erhalten, die _Momentangeschwindigkeit $bold(hk(v)(t))$_, wird die
Position nach der Zeit abgeleitet.
...
entspricht, wird der Satz des Pythagoras verwendet, da die $x$- und $y$-Komponenten den resultierenden Vektor
#hinweis[(Hypothenuse)] bilden.

#definition[$ abs(hk(v)) = sqrt(v^2_x + v^2_y) $
]

=== Umwandlung der Momentangeschwindigkeit
Mit der Momentangeschwindigkeit $hk(v)$ und einem Ortsvektor $hk(r)_A$ kann die _Position $bold(hk(r))$_ zum Zeitpunkt
$t$ ermittelt werden.
#definition[
  #grid(
    columns: (1fr, auto),
...
#grid(
  ...
  [
    #example-block[
      _Ein Teilchen bewegt sich entlang der Bahnkurve \ $hk(r)(t) = vec(cos(t), sin(t))$. Bestimmen Sie die
      Momentangeschwindigkeit des Teilchens zum Zeitpunkt $t$._]
    $
...
#example-block(
  grid(
    align: horizon,
    [
      _Ein Teilchen befindet sich zum Zeitpunkt $t = 0$ im Punkt $hk(r) = vec(1, 0)$. Es bewegt sich mit einer
      Momentangeschwindigkeit von\ $hk(v) = vec(-sin(t), cos(t))$. Wo befindet sich das Teilchen zum Zeitpunkt
      $t = pi/2$ (Viertelumdrehung)?_

      Der gegebene Vektor $hk(r)$ wird als $hk(r)_A$ in die Formel eingesetzt. Mit der Momentangeschwindigkeit kann nun
      integriert werden, um die Position des Teilchens bei einer Viertelumdrehung zu erhalten: Es befindet sich an der
      Position $x = 0, y = 1$.
    ],
    [
      $
...
Für Strecken-Berechnungen reicht eine _skalare Geschwindigkeit_, eine eindimensionale Angabe ohne Vektor. Sie ist der
_Betrag der Momentangeschwindigkeit_. Sie wird auch als $v$ #hinweis[(ohne Vektorpfeil)] geschrieben.
#definition[
  $ abs(hk(v)(t)) = abs(dot(hk(r))(t)) = sqrt((dot(r)_x)^2 + (dot(r)_y)^2 + (dot(r)_z)^2) $
]

Dagegen ist der Betrag der mittleren Geschwindigkeit $abs((Delta hk(r))/(Delta t))$ quasi nutzlos. Viel wichtiger ist
der _Durchschnitt des Betrags der Momentangeschwindigkeit_. Sie gibt die _skalare Durchschnittsgeschwindigkeit_ an --
das, was wir unter Durchschnittsgeschwindigkeit verstehen.
...
Ist die _Momentangeschwindigkeit_ _konstant_, ist sie gleich der _Durchschnittsgeschwindigkeit_. Bewegt sich daher ein
Teilchen zu jedem Zeitpunkt mit $3 "m"\/"s"$, ist auch die Durchschnittsgeschwindigkeit $3 "m"\/"s"$.

*Beispiel: Geschwindigkeit auf dem Kreis*\
#example-block[
  _Ein Teilchen bewegt sich entlang der Bahnkurve $hk(r)_t = vec(cos(t), sin(t))$ bewegt, weist die
  Momentangeschwindigkeit $dot(r)(t) = vec(-sin(t), cos(t))$ auf._

  _a) Bestimmen sie die skalare Geschwindigkeit._

  $ abs(hk(v)(t)) = abs(vec(-sin(t), cos(t))) = sqrt(sin^2(t) + cos^2(t)) = sqrt(1) = underline(1) $
...
Die Beschleunigung $hk(a)(t)$ misst die _Änderung der Momentangeschwindigkeit_. Es gibt zwei Gründe, wieso sich die
Beschleunigung ändern kann:
...
Wenn sich ein Teilchen mit einer konstanten Geschwindigkeit #hinweis[(der Winkel wächst linear an, eine _konstante
  Winkelgeschwindigkeit_)] auf einer Kreisbahn bewegt, ist es eine _gleichförmige Kreisbewegung_. Für eine sich ändernde
Winkelgeschwindigkeit, siehe @ungleichförmige-kreisbewegung.

Die Formel unten wird verwendet, um die _Bahnkurve einer gleichförmigen Kreisbewegung_ zu berechen, siehe
@bahn-teilchen. Die _Winkelgeschwindigkeit $bold(omega)$_ #hinweis[(auch Kreisfrequenz genannt)] gibt an, wie lange eine
Umdrehung im Verhältnis dauert #hinweis[($omega = 2$: Teilchen macht 2 Umdrehungen pro Zeit)]. Die Phase gibt den Winkel
an, wo auf dem Kreis gestartet wird. Häufig wird beim Punkt $vec(R, 0)$ gestartet; hier ist der Winkel $phi = 0°$.
...
#definition(
  grid(
    ...
    [ $ hk(r)(t) = vec(cos(t), sin(t)) $ ],
    [
      Im Einheitskreis ist immer $R = 1$. Eine Umrundung findet immer in einer Zeiteinheit statt, darum $omega = 1$. Das
...
#example-block[
  ...
  #grid(
    ...
    [
      $
        ...
        2) & space phi = arctan(y(0)/x(0)) \
           & space phi = arctan((0"m")/(5"m")) = 0 \
  ...
  #grid(
    [
      + Bahnkurve aus dem oberen Aufgabenteil ableiten, um die Geschwindigkeit zu erhalten. Durch das $omega = 1/s$ gilt
        die Kettenregel, die Sekunde wird aus dem Vektor herausgenommen!
      + Durch eine zweite Ableitung wird die Geschwindigkeitsformel erhalten. Auch hier wird durch die Kettenregel die
        Sekunde erneut herausgezogen.
    ],
    [
      $
...
Ändert sich die Winkelbeschleunigung während der Bewegung, ist sie ungleichförmig. Sie verwendet eine allgemeinere
Formel als die gleichförmige Kreisbewegung.
...
Die Geschwindigkeit der gleichförmigen Kreisbewegung, die _Bahngeschwindigkeit_, zeigt nur an, wie schnell sich das
Teilchen bewegt, aber nicht in welche Richtung.
...
Der _Geschwindigkeitsvektor_ zeigt an, in welche Richtung und wie schnell sich ein Teilchen auf der Kreisbahn bewegt.
...
#example-block[
  ...
  #grid(
    ...
    [
      ...
      + Neue Formel nochmal ableiten, $v dot v/r = v^2/r$ und Minus nach vorne nehmen.
...
Die Beschleunigung zeigt immer zum Mittelpunkt des Kreises.

== Gleichförmige Bewegung
Die gleichförmige Bewegung ist ein _Spezialfall der gleichmässigen beschleunigten Bewegung_, bei welcher _keine
Beschleunigung_ herrscht. Die Geschwindigkeit ist also _konstant_.
...
In einer gleichmässigen beschleunigten Bewegung ist die Beschleunigung eines Teilchens _konstant_. Dadurch können wir
die Geschwindigkeit und Bahnbewegungen durch _Stammfunktionen_ erhalten. Diese sind in der Definition unten bereits
aufgelöst. Dabei ist $Delta t = t - t_0$, wobei häufig $t_0 = 0$.\
...
#definition[
  #grid(
    columns: (1fr,) * 3,
    align: horizon,
    [$ a = (v - v_0) / t = v^2/(2 dot s) $],
    [$ v = v_0 + a dot t $],
    [$ s = v_0 dot t + 1/2 dot a dot t^2 $
    ],
  )
]
...
#grid(
  columns: (1fr, auto),
  [
    Der Schiefe Wurf ist ein Fall der gleichmässigen beschleunigten Bewegung. Dabei wird ein _Teilchen mit einer
...
Beim Schiefen Wurf wird die Anfangsposition $hk(r)_0$ oft auf die $y$-Achse gesetzt. Auch wird häufig der Wurfwinkel
angegeben, womit die Geschwindigkeit berechnet werden kann. Die Anfangsgeschwindigkeit $v_0$ kann auch als skalare
Geschwindigkeit angegeben werden, um eine einzige Zahl für die Geschwindigkeit zu erhalten.
...
möglichst 45° ist #hinweis[($sin(2alpha_0) = 1$)]. Sie kann auch bestimmt werden, wenn statt des Winkels die
Geschwindigkeiten in $x$ und $y$-Richtung oder die Wurfdauer bekannt sind.

#v(-0.25em)
$ x_1 = v_0^2/g dot sin(2 dot alpha_0) quad quad x_1 = (2 dot v_(0, x) dot v_(0,y))/(g) quad quad x_1 = v_(0, x) dot t $
#v(-0.25em)
...
#example-block[
  ...
  #grid(
    [
      ...
      + Die Strecken-Formel ebenfalls in $x$ und $y$ Komponenten zerlegen. Da $hk(r)_0 = 0$, können wir diesen Teil
        weglassen.
      + Uns interessiert, wenn $y = 0$, dann ist der Apfel wieder auf Ausgangshöhe. Dazu setzen wir die $y$-Komponente
        der Streckenformel auf $0$.
...
#table(
  ...
  $ hk(F) = m dot hk(g)\ hk(g) = vec(0, -g) $,
  $ m dot dot.double(hk(r)) = m dot hk(g)\ g = (9.81 plus.minus 0.03) "m"/"s"^2 $,
  $ hk(r)(t) = r_0 + v_0 dot t + 1/2 dot hk(g) dot t^2\ hk(v)(t) = hk(v)_0 + hk(g) dot t $,
)
...
#grid(
  ...
  waagrechter-wurf(scaling: 0.23),
)

*Beispiel: Ende einer Wasserrutsche*\
#example-block[
  _a) Am Ende einer Wasserrutsche soll der Nutzer nach $L = 4"m"$ ins Wasser fallen. Wie hoch muss das Rutschenende
  liegen, wenn der Nutzer mit $v = 8.64 "m"/"s"$ die Rutsche verlässt?_
  $
    h = 1/2 dot g dot (s_x/v_0)^2 = 1/2 dot 9.81 "m"/"s"^2 dot ((4"m")/(8.64 "m"/"s"))^2 = underline(1.05"m")
  $
  _b) Berechnen sie die vektorielle Geschwindigkeit $hk(v)_E$ des Benutzers beim Auftreffen ins Wasser und geben sie den
  Geschwindigkeitsbetrag $v_E = abs(hk(v)_E)$ sowie den Winkel $phi$ an, den die Geschwindigkeit $v_E$ mit der
  Horizontalen einschliesst._
  ...
  #grid(
    ...
    [
      ...
      + Den waagrechten Wurf in eine $x$ und $y$-Komponente aufteilen. In $x$-Richtung bewegt sich der Nutzer mit $v$,
        in $y$ durch den Freien Fall.
      + Vektor $v_E$ bilden
    ...
    $
      1) & space v_x = v = 8.64"m"/"s" \
         ...
         & space v_y = -4.54 "m"/"s" \
      2) & space hk(v)_E = underline(vec(8.64, -4.54) "m"/"s") \
      3) & space abs(v_E) = sqrt(8.64^2 + (-4.54)^2) = 9.76"m"/"s" \
...
Um diese Unterschiede beschreiben zu können, definiert man ein _Bezugssystem_. Ein Bezugssystem ist ein
Koordinatensystem, das in der Perspektive eines Betrachters seinen Ursprung hat. Im obigen Beispiel haben wir das
Bezugsystem der Person $A$ und der Person $Z$. Alle Messungen in einem Bezugssystem finden aus dieser Perspektive statt.
Somit hat der Ball je nach Bezugssystem eine andere Geschwindigkeit.

Bei Rechnungen mit verschiedenen Bezugssystemen werden diese mit einem hochgestellten Index bezeichnet. Es kann so auch
der Ursprung des einen Bezugssystems durch das andere dargestellt werden.
...
Die Geschwindigkeiten und Beschleunigungen für das Bezugssystem $B$ aus dem Bezugssystem $A$ ergeben sich wie folgt.
...
#table(
  ...
  $
    ...
    = overbracket(hk(r)_(0, "Zug") + t dot hk(v)_"Zug", "zurückgelegte Zugstrecke")
    + overbracket(hk(r)^((Z))_(0, "Ball") + hk(v)^((Z))_(0, "Ball") dot t + 1/2 hk(g) dot t^2, "schiefer Wurf")\
...
In der Physik gibt es _vier fundamentale Kräfte:_ Gravitationskraft #hinweis[(Erdanziehungskraft)], elektromagnetische
Kraft #hinweis[(Magnete, elektrisch geladene Objekte)], starke und schwache Kraft #hinweis[(bedeutend auf atomarer
  Ebene)]. Alle anderen Kräfte, die auf Objekte wirken sind _Scheinkräfte/Trägheitskräfte_, siehe @scheinkraft.\
*Beispiele für Scheinkräfte:* Fliehkräfte, Reibung.

In _Inertialsystemen_ wirken keine Scheinkräfte. Sie werden benötigt, um die echten Kräfte von den Scheinkräften zu
separieren. In allen Inertialsystemen gelten dieselben physikalischen Gesetze. Bewegt sich ein zweites Bezugssystem $B$
relativ zum Inertialsystem $I$ geradlinig gleichförmig, ist es ebenfalls ein Inertialsystem.
...
Es ist häufig schwierig, ein "echtes" Inertialsystem zu finden. Zum Beispiel ist auch ein fester Punkt auf der Erde
durch die Erdrotation kein echtes Inertialsystem.

*Beispiel*\
#example-block[
  _
  Während ein Fahrstuhl steht, fällt eine Schraube von der Decke. Der Fahrstuhl ist $3"m"$ hoch und unmittelbar nachdem
  sich die Schraube gelöst hat, beginnt der Fahrzug mit einer Beschleunigung von $4"m"/"s"^2$ nach oben zu
  beschleunigen. Wie lange dauert es, bis die Schraube auf dem Boden des Fahrstuhls ankommt?_
...
Die Newtonschen Gesetze beschreiben, welche _physikalischen Ursachen_ Einfluss auf die Bahn eines Teilchens haben. Durch
sie kommt aus der Kinematik bekannten Grössen die _Kraft_ #hinweis[(Einheit Newton)] und die _träge Masse_ hinzu. Es
gibt drei Newtonsche Gesetze: Das Trägheitsgesetz, Aktionsprinzip und Reaktionsprinzip
...
Ein Objekt kann mehrere Kräfte haben, die gleichzeitig auf es einwirken. Die Summe davon ist die äussere Kraft. Die
Kräfte sind nochmals weiter unterteilt. Diese neuen Kräfte sind alles echte Kräfte, wirken also auch in einem
Inertialsystem.
...
#table(
  ...
  [
    Die Gravitationskraft beschreibt die Anziehungskraft zweier Massen. Durch sie werden alle Objekte zueinander
    hingezogen, auch im Vakuum. Je weiter die Objekte voneinander entfernt sind, desto schwächer wird sie. Im Weltraum
    ist die Gravitationskonstante relevant, siehe @gravitationsgesetz
  ],

  [*Gewichtskraft\ $hk(F)_G$*],
  [
    Kraft, die ein Objekt auf einen Gegenstand wirkt, auf dem seine Masse aufliegt. Je grösser die Masse, desto grösser
    die Gewichtskraft. Die Gewichtskraft ist eine Spezialform der Gravitationskraft, die nur wirkt, wenn Gravitation
    vorhanden ist.
  ],
  ...
  [
    Auf einer schiefen Ebene wird ein Gegenstand durch die Gravitation nach unten gezogen. Sie wird meistens in eine
    senkrechte und zur Ebene parallele Komponente aufgeteilt. Ersteres drückt den Gegenstand gegen die Ebene und wird
    von der Normalkraft kompensiert. Die parallele Komponente besteht aus der Gravitationskraft und zieht den Gegenstand
    die Ebene herab. Die Hangabtriebskraft ist die Summe aus Gravitationskraft und Normalkraft.
  ],
  [*Strömungs-\ widerstand\ $hk(F)_"ström"$*],
  [
    Tritt auf, wenn ein Körper durch Luftwiderstand abgebremst wird. Ebenfalls eine Reibungskraft, da der Wind aber
    meist nicht als Teil eines Systems betrachtet wird, gilt sie häufig als äussere Kraft.
  ],
  [*Rückstellkraft\ $hk(F)_R$*],
  [
    Spannt man eine Feder, wirkt eine Kraft, die die Feder wieder in ihre Ruhelage bewegt -- die Rückstellkraft
  ],

  table.cell(rowspan: 4, rotate(-90deg, reflow: true)[Innere Kräfte]),
  [*Reibungskräfte*],
  [
    Treten auf, wenn sich zwei Körper mit unterschiedlicher Geschwindigkeit berühren. Verlaufen entgegen der
    Bewegungsrichtung der Körper. Werden in verschiedene Arten unterteilt:
  ],
  ...
  [
    Tritt auf, wenn ein Körper mit Rädern auf einem anderen Körper platziert wird und damit über einen Körper rollt. Der
    Gleitreibung sehr ähnlich, aber meist quantitativ deutlich geringer.
  ],
  [*Haftreibung\ $hk(F)_"Haft"$*],
  [
    Um einen Körper, der auf einem anderen ruht, in Bewegung zu versetzten, braucht es eine gewisse Minimalkraft. Wirkt
    eine Kraft $<$ Haftreibungskraft ein, wird sie dadurch kompensiert und der Körper bewegt sich nicht. Sobald sich ein
    Körper bewegt, gibt es keine Haftreibungskräfte mehr.
  ],
)

Die _äussere Kraft_, die auf ein Teilchen einwirkt, kann von Zeit, Position und Geschwindigkeit des Teilchens abhängen.
Sie wird deshalb oft als Funktion definiert. Dies ist bei der Bewegungsgleichung relevant #hinweis[(siehe
  @bewegungsgleichung)].

#definition[$ F_"äussere" (hk(r), hk(v), t) $]

=== Kraftvektoren
Kräfte werden als _Vektoren_ definiert und haben eine Richtung und eine Länge. Das ist relevant, wenn ein Kraftvektor
einen Winkel hat. Ein solcher Vektor wird durch seine Länge $F$ und seine Richtung #box(
  inset: (y: -1em),
  $vec(cos alpha, sin alpha)$,
) definiert. Kraftvektoren können auch in eine $x$ und $y$-Komponente aufgeteilt werden.
...
#definition(
  grid(
    ...
    [
      ...
      _$bold(vec(cos alpha, sin alpha))$:_ Richtung des Vektors, basierend auf dem Winkel\
      _$bold(F_x\, F_y)$:_ $x$ bzw. $y$-Komponente des Vektors
    ],
  ),
)

Der Nullpunkt des Koordinatensystems ist meist unten links im Schaubild. In diesen Koordinatensystemen sind _alle
Kräfte, die nach links oder unten zeigen, negativ_.

*Beispiel*\
#example-block[
  _An einem Lastarm $A$ wird eine Masse $m$ befestigt und mit einem Seil $S$ gemäss der rechts stehenden Skizze nach
  oben gezogen. Die Masse wird nicht beschleunigt, also ist $hk(F)_"äussere" = 0$._
  #grid(
    align: horizon,
    $
...
== Statik
Im Gegensatz zu der Kinematik, die sich auf die Lehre von den Bewegung der Körper handelt, beschäftigt sich die Statik
mit Objekten, die sich im ruhenden Zustand befinden. Es kommt zum _Kräftegleichgewicht_, in welchem sich sämtliche auf
ein Objekt wirkenden Kräfte kompensieren.
...
#quote(attribution: "1. Newtonsches Gesetz (Trägheitsgesetz)")[
  In einem Inertialsystem bewegt sich ein Teilchen, auf das keine äussere Kraft wirkt, geradlinig gleichförmig. Das
  Teilchen verharrt entweder in Ruhe oder seine Bahn verläuft entlang einer Geraden mit konstanter Geschwindigkeit.
]
#v(-0.75em)
...
Damit sich die Kiste nicht bewegt, muss die Normalkraft $F_N$ des Bodens diese ausgleichen.
...
#grid(
  ...
  [
    Weil die Normalkraft immer senkrecht auf die Kontaktfläche mit der Ebene wirken muss, heben sich auf der_
    schiefen Ebene_ die Normalkraft und die Gewichtskraft nicht mehr auf. Auf einer Schiefen Ebene ohne Reibung ist
    $F_N > F_G$, darum wirkt die Hangabtriebskraft $F_H$, siehe @schiefe-ebene-ohne-reibung.
  ],
  image("img/schiefe_ebene.png"),
)

Gleitet ein Objekt über den Boden, gilt zusätzlich zur Gewichts- und Normalkraft die Gleitreibung $F_"gleit"$. Die
ersten beiden Kräfte heben sich allerdings gegenseitig auf. Die äussere Kraft berechnet sich also
$
  F & = F_G + F_N + F_"gleit" quad = quad 0 + F_"gleit" = F_"gleit"
$

== 2. Newtonsche Gesetz: Aktionsprinzip
#v(-1.25em)
#quote(attribution: "2. Newtonsches Gesetz")[
  In einem Inertialsystem ist die Beschleunigung, die ein Körper erfährt, proportional zur auf ihn einwirkenden äusseren
  Kraft.
]
#v(-0.75em)
Es muss eine grössere Kraft aufgewendet werden, um einen schweren Körper genau so stark zu beschleunigen wie einen
leichten. Die _träge Masse $bold(m)$_ eines Körpers widersetzt sich der Beschleunigung. Sie hat die _Einheit
$bold("kg")$_. Je grösser die träge Masse eines Objekts ist, umso mehr Kraft muss man aufwenden, um einen Körper auf ein
gewisses Mass zu beschleunigen.

#definition[
  $ hk(F)_"äussere" = m dot hk(a) $
]

Die träge Masse stellt eine Beziehung zwischen der auf einen Körper wirkende äussere Kraft und seiner Beschleunigung
her. Zusätzlich gibt es noch die _schwere Masse_, das Gewicht eines Körpers, welches auf der Waage abgelesen werden
kann. Die Relativitätstheorie beweist aber, dass träge und schwere Masse quasi äquivalent sind, und deswegen einfach von
_Masse_ gesprochen werden kann.

=== Bewegungsgleichung <bewegungsgleichung>
Die _Bewegungsgleichung_ beschreibt die Bahn eines Teilchens. Sie basiert auf dem zweiten Newtonschen Gesetz. Hier wird
$F_"äussere"$ definiert als Funktion, die von der Zeit $t$ #hinweis[(z.B. eingeschalteter Elektromagnet)], der Position
$hk(r)$ und der Geschwindigkeit $hk(v)$ #hinweis[(z.B. bei Reibung)] abhängig ist. Weil sich $hk(a)$ auch als
$dot.double(hk(r))$ schreiben lässt, hängen beide Seiten von denselben Variablen $r$ und $t$ ab.

#definition[
  $ m dot dot.double(hk(r)) = hk(F)(hk(r), dot(hk(r)), t) $
]

Hier erhalten wir eine _Differentialgleichung_ #hinweis[(genauer: Differentialgleichung 2. Ordnung)], eine Gleichung, in
welcher die Unbekannte keine Variable, sondern eine Funktion ist. In der Physik haben die Differentialgleichungen der
Bewegungsgleichung typischerweise eine _allgemeine Lösung_, die von _zwei Parametern_ abhängt. Diese beiden Parameter
sind die _Anfangswerte $bold(hk(r)_0\, hk(v)_0)$_ bzw. $hk(r)_0\, dot(hk(r))_0$. Werden diese als Parameter in der
Differentialgleichung fixiert, erhält man die _spezielle Lösung der Differentialgleichung_.
...
#example-block[
  ...
  #grid(
    ...
    [
      ...
      + Die Beschleunigung wird durch die Gewichtskraft\ $F_G = m dot g$ der Steine bestimmt
  ...
  Ein sehr schwerer Stein bewegt sich wie ein Stein ohne Luftreibung #hinweis[($hk(g) + 0$)]. Bei einem sehr leichten
  Stein wird sie irgendwann sehr viel grösser als $hk(g)$ #hinweis[($hk(g) + infinity$)]. Der Stein wird rasch auf
  $0"m"/"s"$ abgebremst und fällt zu Boden. Der sehr leichte Stein bewegt sich also nicht nach vorne, während der sehr
  schwere Stein die Luftreibung quasi ignoriert.
]
...
#grid(
  align: horizon,
  [
    Auf der schiefen Ebene ohne Reibung setzt sich die äussere Kraft aus zwei Kräften zusammen: Der Gewichtskraft
    $hk(F)_G$ und der Normalkraft $hk(F)_N$. Da sich die beiden Kräfte nicht mehr ausgleichen, kommt zur äusseren Kraft
    die _Hangabtriebskraft_ hinzu. Sie ist die Summe der beiden Kräfte und somit gleich der totalen äusseren Kraft.
  ],
...
Die Gewichtskraft $hk(F)_G$ lässt sich in zwei Komponenten einteilen: Die Hangabtriebskraft $hk(F)_H$, welche die Ebene
herunterzeigt und die negative Normalkraft $-hk(F)_N$, welche senkrecht von der schiefen Ebene weg weist.
...
#definition[
  $ F_H = m dot g dot sin alpha\ F_G = m dot g dot cos alpha $
]
...
#definition[$
  r(t) = r_0 + 1/2 a dot t^2 space = space r_0 + hk(F)_H/(2m) dot t^2 space = space r_0 - (g dot sin alpha)/2 dot t^2
$]
...
#example-block[
  #grid(
    [
      _Zwei Körper mit Massen $m_1 = 1.5"kg"$ und $m_2 = 1"kg"$ sind reibungsfrei über eine Rolle mit einem Seil
      verbunden. $m_1$ liegt auf einer schiefen Ebene mit $alpha = 30°$. $m_2$ hat keinen Kontakt mit der schiefen
      Ebene. Bestimme die Beschleunigung von $m_1$. Bewegt er sich die Ebene hoch oder hinunter?_
    ],
    image("img/schiefe_ebene_rolle.png"),
  )


  Wichtig zu wissen ist, dass das gespannte Seil diesselbe Zugkraft $F_Z$ und damit auch diesselbe Beschleunigung $a$
  ...
  #grid(
    ...
    [
      ...
      + Bewegungsgleichungen für beide Körper aufstellen. $F_Z$ ist bei $m_2$ minus, da der Körper nach unten gezogen
        wird und kürzt sich weg.
...
#table(
  ...
  align: horizon,
  table.header([Äussere Kraft], [Bewegungsgleichung], [Allgemeine Lösung]),
  $ hk(F)_"äussere" = hk(F)_H\ = -m dot g dot sin(alpha) dot hat(r)_"Ebene" $,
  $ m dot dot.double(r) = -m dot g dot sin(alpha) $,
  $ h(t) = \ h_0 + v_0 dot t - 1/2 dot g dot sin(alpha) dot t^2\ v(t) = v_0 - g dot sin(alpha) dot t $,
)
...
aus Gleit- oder Rollreibungskraft $hk(F)_"gleit"$ oder $hk(F)_"roll"$, je nach dem ob der Körper Räder besitzt. Die
Reibungskraft ist immer _entgegen der Geschwindigkeit_ gerichtet und proportional zu $abs(hk(F)_N)$.
...
#definition[
  #grid(
    ...
    $,
    [
      _$bold(minus.plus)$:_ Bewegt sich der Körper den Hang hinunter, ist das Vorzeichen $minus$. Bewegt er sich die
      schiefe Ebene hoch, ist es $plus$.
    ],
  )

]

Zusätzlich gibt es noch die Haftreibung. Sie beschreibt den Widerstand, bis sich ein Körper in Bewegung setzt. Ist die
Summe aller anderen Kräfte, die auf das Objekt wirken kleiner als die _maximale Haftreibung_, kompensiert sie die Kraft.
...
#table(
  ...
  align: horizon,
  table.header([Äussere Kraft], [Bewegungsgleichung], [Allgemeine Lösung]),
  $ hk(F)_"äussere" = hk(F)_H + F\ = -m dot g dot sin(alpha) dot hat(r)_"Ebene" $,
  $ m dot dot.double(r) = -m dot g dot sin(alpha) $,
  $ h(t) = \ h_0 + v_0 dot t - 1/2 dot g dot sin(alpha) dot t^2\ v(t) = v_0 - g dot sin(alpha) dot t $,
)

=== Federpendel mit Hooksches Gesetz (Feder-Kraft-Gesetz)
Bei einer Feder gilt die Rückstellkraft $F_R$. Sie ist abhängig von der Spannung der Feder $s$, also gilt $F_R (s)$. In
Ruhelage herrscht ein Kräftegleichgewicht, also $F_R (0) = 0$. Um das Verhalten der Feder zu beschreiben, reicht die
erste Ableitung der Federkraft $F'_R (0)$, diese wird als _Federkonstante $bold(D)$_ bezeichnet. $D$ ist abhängig von
dem Material und der Geometrie der Feder, darum sie wird meist experimentell bestimmt. Die Rückstellkraft wirkt immer in
die Gegenrichtung der Feder; da für die Federkonstante positive Zahlen angenehmer sind, lautet das _Hooksche Gesetz_:
...
#grid(
  ...
  [
    ...
    Differenzialgleichung nötig, siehe @bewegungsgleichung. Die Spannung der Feder $s$ ist wie die Beschleunigung $a$
    von der Zeit $t$ abhängig sind; es gilt $a(t) = dot.double(s)(t)$.
  ],
  [
    $
      m dot a = -D dot s\
...
Daraus ergibt sich auch die Periodendauer $T$ des Pendels. Sie gibt an, wie lange es dauert, bis die Feder wieder an der
Ausgangsposition angelangt ist.
#definition[
  $ T = 2pi dot sqrt(m/D) $
]
...
#definition[
  $ v(t) = -s_0 dot sqrt(D/m) dot sin(sqrt(D/m) dot t) + v_0 dot cos(sqrt(D/m) dot t) $
]
...
#example-block[
  ...
  Die Gewichtskraft der Masse $F_G = m dot g$ und der Rückstellkraft der Feder $F_R = D dot x_0$ #hinweis[($x_0$:
  ...
  Beim Start des Experiments sind die Kräfte (noch) unverändert. Nun kommt die angewendete Kraft $F$ hinzu. Da die
  äussere Kraft vorher $0$ war, ist $F$ identisch mit der äusseren Kraft. Die Beschleunigung der Masse beträgt $a = F/m$
  #hinweis[(Umstellung des 2. Newtonschen Gesetz)].
  ...
  Durch die Beschleunigung bewegt sich die Masse minimal nach unten: $a = -F/m$. Wird $h$ positiv, bewegt sich das
  Gewicht nach oben und die Rückstellkraft wird geringer. Dasselbe gilt umgekehrt.\
  ...
  */
  ...
  Es gilt $h(0) = 0$ und weil sich die Masse zu Beginn nicht bewegt $dot(h)(0) = 0$
  ...
  Beim Ziehen an der Konstruktion muss der obere Faden $F + m dot g$ tragen, der untere nur $F$. Der obere Faden kann
  also noch $F_"reiss" - m dot g$ tragen, bevor er reisst.

  Gilt $F > F_"reiss"$, reisst der untere Faden sofort. Da die Kraft einige Zeit benötigt, um den oberen Faden zu
  erreichen, können nicht beide Fäden gleichzeitig zerreissen. Auf den oberen Faden wirkt danach nur noch die
  Gewichtskraft der Masse. Ist der Faden noch nicht gerissen, kann er das auch nicht mehr.\
  Gilt $F_"reiss" - m dot g < F < F_"reiss"$ reisst der untere Faden nicht, weil $F < F_"reiss"$. Der obere Faden reisst
  aber, weil $F_"reiss" - m dot g < F$
]
...
#table(
  ...
  align: horizon,
  table.header([Äussere Kraft], [Bewegungsgleichung], [Allgemeine Lösung]),
  $ hk(F)_"äussere" \ = hk(F)_H -D dot s $,
  $ m dot dot.double(s) = -D dot s $,
  $ h(t) = h_0 + v_0 dot t - 1/2 dot g dot sin(alpha) dot t^2\ v(t) = v_0 - g dot sin(alpha) dot t $,
)

=== Fadenpendel
Wird eine Masse $m$ an einen Faden der Länge $l$ gehängt, können kleine Auslenkungen der Masse als Spezialfall des
Federpendels angesehen werden. Hier lässt sich die Federkonstante mathematisch bestimmen.
...
Sitzt man in einem Ruderboot, drückt man mit einem Ruderschlag mit der Kraft $F_R$ das Wasser nach hinten. Die Bewegung
des Wassers drückt das Boot mit der Kraft $-F_R$ nach vorne.


Beim Seilziehen wird das Seil von Person 1 mit einer Kraft $F_1$ gezogen. Auf der anderen Seite zieht Person 2 das Seil
mit Kraft $F_2$. Person 1 wird mit der Kraft $-F_2$ zur Person 2 hingezogen und umgekehrt. Um die Kraft ausüben zu
können, muss sich Person 1 mit der Kraft $-F_1$ gegen den Boden stemmen. Damit Person 1 an Ort und Stelle bleibt, muss
der Boden die Kraft $+F_1$ ausüben.
...
#example-block[
  ...
  Wenn sich die Akrobatin auf dem Seil steht, drückt sie mit $hk(F)_G$ das Seil nach unten. Das Seil verteilt diese
  Kraft mit $hk(F)_z$ und $hk(F)_z'$ auf die Gebäude links und rechts. Das Seil hängt am meisten durch, wenn die
  Akrobatin genau in der Mitte des Seils steht.
  #grid(
    columns: (1fr, auto),
    [
...
== Kreiskräfte
- _Zentrifugalkraft:_ Fliehkraft, drückt einen Körper auf einer Kreisbewegung nach aussen. Sie zeigt senkrecht vom
  Kreismittelpunkt weg.
- _Zentripetalkraft:_ Kraft, die einen Körper auf eine Kreisbahn zwingt. Sie zeigt gerade auf den Kreismittelpunkt und
  somit das genaue Gegenteil der Zentrifugalkraft.
...
#grid(
  ...
  kreiskraft(scaling: 1),
)
#definition[
  $ F_Z = m dot v^2/r quad quad quad F_T = m dot dot(v) $
]
...
#example-block[
  ...
  der $x$-Achse und der Kraft der Bremsbeschleunigung in der $y$-Achse ergibt. Diese Kraft muss die Haftreibung
  $F_(H, min)$ mindestens kompensieren. Die Kraft $F_Z$ ist bei der grössten Geschwindigkeit $v_0$ am höchsten.
  #grid(
    columns: (1fr, auto),
    [
    ...
    $
      ...
      3) & space F_"äussere" = sqrt(F_Z^2 + F_a^2) \
         & space F_"äussere" = sqrt((-0.625"N")^2 + (-0.2"N")^2) \
         & space F_"äussere" = 0.656"N" \
...
#grid(
  columns: (1fr, 1fr),
  [
    + Die Umlaufbahnen aller Planeten sind ellipsenförmig. Die Sonne steht in einem der Brennpunkte der Ellipse.
...
In der Kosmologie ist die Kreisbahn durch die Zentripetalkraft der Gravitation der Himmelskörper vorgegeben. Diese haben
die Form
#definition(
  grid(
    align: horizon,
...
#grid(
  [
    ...
    - _Inertialsystem $bold(I)$_, in welchem das 2. Newtonsche Gesetz gilt.
    - _Bezugssystem $bold(B)$_ des Beobachters. Kann ebenfalls ein Inertialsystem sein. Koordinatensystem zeigt in
      dieselbe Richtung wie $I$.
...
+ _Vom Ursprung des Inertialsystems aus gesehen: $bold(hk(r)^((I))_(T) (t))$_ \
  ...
  _$m dot.double(hk(r))^((I))_T = hk(F)^((I))_"äussere"$_.

+ _Aus der Sicht des Beobachters: $bold(hk(r)^((B))_T (t))$_ \
  Das Teilchen hat sich vom Ursprung von $B$ entlang $hk(r)^((B))_T (t)$ zur aktuellen Position bewegt. Da sich der
  Beobachter $B$ nicht in einem Inertialsystem befinden muss, wissen wir #hinweis[(noch)] nicht, welche
  ...
  #v(-0.5em)
  $
    m_T dot dot.double(hk(r))^((B))_T quad = quad m_T dot dot.double(hk(r))^((I))_T - m_T dot dot.double(hk(r))^((I))_B
    quad = quad hk(F)^((I))_"äussere" + hk(F)^((B))_"Schein"
  $

Die Scheinkraft hängt von der Masse des beobachteten Teilchens und der auf den Beobachter #hinweis[(nicht das
  Teilchen!)] wirkendenden Beschleunigung ab. Beschreibt der Beobachter also mehrere Teilchen, werden alle Teilchen um
diesselbe Scheinkraft korrigiert, wenn die Teilchen diesselbe Masse haben.

Die Scheinkraft kann auch als das _negative der auf den Beobachter wirkenden äusseren Kraft_ definiert werden. Das
klappt, weil die Bahn des Beobachters $hk(r)^((I))_B$ aus einem Inertialsystem beschrieben wurde und damit die
Newtonische Bewegungsgleichung $m_B dot dot.double(hk(r))^((I))_B = hk(F)^((I))_B$ erfüllt. Damit wird also kompensiert,
dass auf den Beobachter selbst Kräfte wirken.
...
Alle physikalischen Körper reagieren exakt auf dieselbe Weise auf die Gravitation. Im Newtonschen Gravitationsgesetz ist
die durch die Masse $M$ #hinweis[(z.B. Planet Erde)] auf einen Körper mit Masse $m$ #hinweis[(Gegenstand)] ausgeübte
Gravitationskraft proportional zu $m$ des Körpers. Durch das zweite Newtonsche Gesetz $m dot.double(hk(r)) = hk(F)$
fällt die Masse $m$ weg.
...
Die durch die Masse $M$ bewirkte Beschleunigung anderer physikalischer Körper hängt also nur von deren relativer
Position zu $M$ und und nicht von ihren individuellen Massen ab.

Ein Teilchen und ein Beobachter befinden sich im gleichen konstanten Gravitationsfeld $hk(g)$ #hinweis[(nach unten
  gerichtet)]. Zusätzlich wirkt auf das Teilchen eine Gesamtkraft $hk(F)$. Der Beobachter selbst ist nur der Gravitation
ausgesetzt.
...
#grid(
  ...
  align: horizon,
  [
    Die Scheibe dreht sich mit konstanter Winkelgeschwindigkeit $omega$. Ein Beobachter $B$ dreht sich mit der Schiebe.
    Das Teilchen $T$ bewegt sich mit gleichbleibender Geschwindigkeit $hk(v)^((B))_T$. Dabei erfährt es die äussere
    Kraft bestehend aus der _Zentripetalkraft $bold(hk(F)_Z)$_ und _Corioliskraft $bold(hk(F)_C)$_.
...
_Wärmeenergie:_ Durch innere Kräfte erzeugte Energie wie Reibung oder Hitze. Kann nur schlecht in potentielle oder
kinetische Energie umgewandelt werden.


=== Energieerhaltungssatz
Die oben genannten Arten von Energien lassen sich ineinander umwandeln. Dabei geht aber nie Energie verloren, es wird
immer alle Energie in eine oder mehrere andere Energieformen umgewandelt. Der _Energieerhaltungssatz_ sagt aus, dass die
Summe der Energien _immer konstant bleibt_. Er ist äquivalent zum 2. Newtonschen Gesetz. Unten abgebildet ist der
Energieerhaltungssatz _ohne Reibung._
...
Energieerhaltungssatz für beide Teilchen zusammen. $E_"pot"$ hängt von der Position $x$ der Teilchen ab und wird auch
als _Potential $bold(V(x))$_ bezeichnet.

$ E_"tot" = E_"kin" + E_"pot" = sum_(i = 1)^2 1/2 m_i dot v_i^2 + sum_(i = 1)^2 V(x_i) $

In _wechselwirkenden Mehrteilchensystemen_ haben die einzelnen Teilchen _keine potenzielle Energie mehr_. Nur für das
Gesamtsystem kann ein Energieerhaltungssatz aufgestellt werden. Die potentielle Energie "liegt im Feld" und wird durch
die _Wechselwirkung $bold(U)$_ definiert.

#definition[
  $ E_"tot" = E_"kin" + E_"pot" = 1/2 m_1 dot dot(x)_1^2 + 1/2 dot m_2 dot dot(x)_2^2 + U(x_1 - x_2) $
]

Die Wechselwirkung $U$ hängt meist nur vom Abstand der Teilchen ab. Für verschiedene Szenarien gibt es unterschiedliche
Formeln. Bei mehreren Teilchen muss immer das Teilchen mit den grösseren Koordinaten zuerst stehen.
...
#table(
  ...
  [Gravitation 1 Teilchen\ #hinweis[(Sonne zu Planet)]], $ E_"pot" = -(Gamma dot M dot m)/abs(hk(r)) $,
  [Gravitation 2 Teilchen\ #hinweis[(2 Planeten)]], $ E_"pot" = -(Gamma dot m_1 dot m_2)/(abs(hk(r)_1 - hk(r)_2)) $,
)

Da die Teilchen aufeinander wirken, sind auch die Bewegungsgleichungen der Teilchen von der Position beider Teilchen
abhängig.
$
  m_1 dot dot.double(x)_1 = F_1(x_1, x_2) quad quad quad m_2 dot dot.double(x)_2 = -F_1(x_1, x_2)
$



== Konservative Kräfte <konservative-kräfte>
Als konservative Kräfte gelten Kräfte, deren Kraft unabhängig von der erbrachten Arbeit ist. Beispielsweise ist die
Gravitationskraft konservativ, da sie immer diesselbe ist, egal wie viel Arbeit erbracht wird. Das Gegenteil sind
_dissipative Kräfte_, wie z.B. Reibung. Sie verrichten Arbeit proportional zum Weg. Durch Luftreibung kehrt ein
Bungee-Springer nicht mehr zur Ausgangshöhe zurück.

Eine dissipative Kraft scheint den Energieerhaltungssatz zu verletzten. Die Energie, die z.B. durch Reibung "verloren"
geht, wird aber auf atomarer Ebene in sogenannte _innere Energie_ umgewandelt. Sie lässt sich in Form von Temperatur
indirekt messen.
...
+ Wenn die partielle Ableitung der Komponenten den Wert $-hk(F)$ ergibt. Diese Funktion ist der _Gradient
  $bold(nabla V(hk(r)))$_ #hinweis[(Nabla-Operator)]. Somit gilt
#definition[
  $
...
#example-block[
  _Auf einer schiefen Ebene steht ein Körper mit der Masse $m = 3"kg"$ auf der Höhe $h_0 = 3"m"$. Er rutscht die Ebene
  hinunter bis zum Punkt $h_1 = 2"m"$. Hier hat er eine Geschwindigkeit von $v_1 = 2"m"/"s"$. Wie viel innere Energie
  wird produziert?_
...
Energieerhaltungssatz. Fügt man einem System von aussen Energie zu, _verrichten wir Arbeit am System_. Wird dem System
Energie entzogen und der Aussenwelt hinzugefügt, _verrichtet das System Arbeit._ Die Arbeit hat die _Einheit Joule_ und
das Zeichen _$bold(W)$_.
...
#example-block[
  ...
  #grid(
    ...
    [
      ...
      + Die Arbeitsformel aufstellen und $F$ mit $F_R$ und $F_G dot sin alpha$ ersetzen. $s$ kann ebenfalls mit
        $h/(sin alpha)$ ersetzt werden.
...
=== Leistung
Leistung ist Arbeit pro Zeit. Je höher die Leistung, desto mehr Energie wurde pro Zeiteinheit verrichtet. Die Leistung
ist eine Ableitung der Arbeit und wird in der Einheit _Watt_ gemessen, was $1 "J"/"s"$ entspricht.

#definition[$ P(t) = dif/(dif t) W(t) = dot(W)(t) quad quad P = dot(W) $]

Je schneller sich der Körper bewegt, desto grösser die Leistung, die auf den Körper einwirkt. Das zeigt sich
beispielsweise an der _Leistung an der schiefen Ebene_:
...
= Kraft und Impuls
In einem _abgeschlossenen System_ wirken nur Kräfte, deren Ursachen innerhalb dieses Systems liegen. Für ein
abgeschlossenes System gilt: Die Summe aller äusseren Kräfte, die auf Teilchen in einem abgeschlossenen System wirken,
ist gleich Null.
...
== Impuls
Der _Impuls $bold(hk(p))$_ beschreibt den Bewegungszustand eines Körpers, basierend auf seiner Masse und
Geschwindigkeit. Haben zwei Teilchen diesselbe Geschwindigkeit, aber unterschiedliche Massen, hat das Teilchen mit der
grösseren Masse den höheren Impuls. Er hat die Einheit $bold(("kg" dot "m") / "s" = "Ns")$.
...
Die _Schwerpunktgeschwindigkeit $bold(u)$_ eines Systems ist die Geschwindigkeit, die ein Teilchen hätte, wenn es den
Impuls und die Masse aller Teilchen im System besitzen würde.
...
Je nach Geschwindigkeit und Masse kann die Kraft, die beim Zusammenstoss zwischen den Körpern übertragen wird,
variieren. Die _mittlere übertragene Kraft $bold(dash(F))$_ ist die durchschnittliche Kraft, die beim Zusammenstoss
...
#example-block[
  #grid(
    align: horizon,
    [
      _Zwei Kugeln werden nebeneinander an jeweils einer Schnur aufgehängt, dass sie sich im Ruhezustand berühren. Beide
      Kugeln werden nun in entgegengesetzte Richtungen mit einem Winkel von $alpha = 40°$ ausgelenkt und anschliessend
      losgelassen. Die Kugeln prallen aneinander ab und erreichen beide wieder ihre Ausgangshöhe._
    ],
    image("img/pendel.png"),
  )
  #v(-0.75em)
  _Die Kugeln haben die Masse $m = 0.1"kg"$ und einen masselosen Faden mit der Länge $l = 0.2"m"$. Die Stossdauer wird
  mit $Delta t = 1 dot 10^(-4)"s"$ gemessen._

  _a) Bestimmen sie die Höhe $Delta h$, aus der die Kugeln losgelassen werden._

  Die Höhe kann mit Pytagoras berechnet werden.
  $
  ...
  $
    E_"kin" = E_"pot" quad => quad 1/2 dot m dot v^2 = m dot g dot Delta h quad => quad v = sqrt(2g dot Delta h) \
  ...
  #grid(
    ...
    [
      ...
      + Da die Kugeln aus gleicher Höhe fallen gelassen werden, müssen sie auch diesselbe Geschwindigkeit haben. Sie
        bewegen sich aufeinander zu, deswegen muss ein Term negativ sein.
  ...
  Nach dem Stoss haben die Geschwindigkeiten denselben Betrag, aber entgegengesetzte Richtungen. Wir wissen, dass die
  Kugeln wieder ihre Ausgangshöhe erreichen. Damit gilt $v' = -v = -0.96 "m"/"s"$
  ...
  #grid(
    columns: (1fr, auto),
    [
      + Die Kugeln bremsen auf $0$ ab und beschleunigen in entgegengesetzter Richtung. Dafür ist der doppelte Impuls
        nötig.
  ...
  _f) Wie gross ist die Impulsänderung des Gesamtsystems bestehend aus beiden Pendeln? Ist der Gesamtimpuls des Systems
  enthalten?_
  #grid(
    columns: (1fr, auto),
    [
...
- Die Kräfte, die auf den Körper wirken, lassen sich in Totalkraft und Drehmoment (um einen Bezugspunkt) zusammenfassen. Für jede beliebige Kraftverteilung müssen wir nur diese beiden Grössen ausrechnen
